...
class TransferException extends CI_Controller {
	...
	public function facilityDiff(){
        ...
        if ($cond['product_type'] == 'supplies' && !$this->helper->chechActionList(array('suppliesTransferManager'))) {
        	die("无耗材权限");
        }
	   	$out = $this->transferinventory->getFacilityDiff($cond['facility_id'],$cond['product_type']);
	   	if (isset($out['error_info'])) {
	   		die("无法获取数据" . $out['error_info']);
	   	}
		$cond['facility_list'] = $facility_list;
