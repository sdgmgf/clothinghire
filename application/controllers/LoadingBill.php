...
class LoadingBill extends CI_Controller {
	function __construct()
	{
	    ...
	    $this->helper->chechActionList(array('loadingBill','TILoadingBill'),true) ;
  ...
  {
  	$this->helper->chechActionList(array('loadingBill'), true);
	  $cond = $this->initData();
    $cond['from_p'] = 'INDEX';
    ...
    $asn = $this->purchase->getAsn($cond['facility_id'], $cond['po_date'], 'goods', $cond['facility_type'], $cond['area_type']);
  ...
  public function initData(){
  	...
  	if(empty($area_list) || ! isset($area_list['data']) || count($area_list['data']) < 1) {
  	...
  	if(count($area_list['data']) > 1) {
  		$initData['error'] = 'error';
  		$initData['error_info'] = '账号拥有多个大区权限';
  		return $initData;
  	}
  	
    $area = $area_list['data'][0];
  	$out = $this->common->getPurchasePlaceByAreaId($area['area_id']);
  	$facility_list = $out['data'];
  	if (empty($facility_list)) {
  		$initData['error'] = 'error';
  		$initData['error_info'] = '无采购地';
  		return $initData;
  	}
  	$initData['facility_list'] = $facility_list;
