...
class ProductEdit extends CI_Controller {
    ...
    public function getFulfillRevenueList() {
    	$this->helper->chechActionList(	array('getFulfillRevenue'),true);
    	$product_id = $this->getInput("product_id");
    	$out = $this->productmodel->getFulfillRevenueList($product_id);
    	echo json_encode(array('res'=>'success', "fulfill_revenue_list" => $out['fulfill_revenue_list']));
    }
    
    //修改履约收入
    public function upsertProductRevenue(){
    	$this->helper->chechActionList(	array('upsertFulFillRevenue'),true);
    	$params['product_id'] = $this->getInput('product_id');
    	$params['start_time'] = $this->getInput('start_time');
    	$params['warehousing_revenue'] = $this->getInput('warehousing_revenue');
    	$params['trunk_revenue'] = $this->getInput('trunk_revenue');
    	$params['shipping_revenue'] = $this->getInput('shipping_revenue');
    	$params['supplies_revenue'] = $this->getInput('supplies_revenue');
    	$params['production_cost_sharing'] = $this->getInput('production_cost_sharing');
    	$params['management_cost_sharing'] = $this->getInput('management_cost_sharing');
    	$params['other_revenue'] = $this->getInput('other_revenue');
    	$params['other_note'] = $this->input->post('other_note');
    	$params['fulfill_revenue'] = $this->getInput('fulfill_revenue');
    
    	$out = $this->productmodel->upsertProductRevenue($params);
    	if($this->helper->isRestOk($out)){
    		echo json_encode(array('success'=>'success'));
    	}else{
    		echo json_encode(array('success'=>'fail','error_info'=>!empty($out['error_info'])?$out['error_info']:'服务器内部错误'));
