...
<body>
<?php if( !empty($area_list) && count($area_list)>1 ){ ?>
<p class="remind">拥有多个大区权限的账号不可操作</p>  
<?php } ?>
       ...
       <div class="modal-dialog" style="display: inline-block; width:70%;">
        <div class="modal-content">
          ...
          <div class='row'>
                    ...
                    <label for="modal_unit_code_desc" style="text-align: right;">单位：</label><input type="text" style="color: red" disabled="disabled" id="modal_unit_code_desc">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body ">
        <div class='row'>
            ...
            <label for="stock_up_product_name">商品：</label><input type="text" style="width:30%; height: 100%" id="stock_up_product_name" name="stock_up_product_name">
        </div>
        <div class="row"><button class="btn btn-success btn-lager" id="add_stock_up_btn" style="margin-left: 50px" >提交</button></div>
      </div>
      <div class="modal-footer">
      </div>
    </div>
    ...
    function createTableHtml(){
        ...
        getPurchaseOrderItemList(data).done(function(data){
            if( data.success === 'success' ){
                ...
                console.log(purchase_place_list);
                $.each(purchase_place_list,function(){
                    var self = this;
