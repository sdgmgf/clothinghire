...
<body>  
    <div name="scandiv"  id="scandiv" class="container-fluid" style="margin-left: -18px;padding-left: 19px;" >
...
function load_tracking_number() {
	if(!is_extension_load()){
		alert('请越狱下载 google extension:Allow clipboard');
		return;
	}
...
function addTrackingNumber(tracking_number) {
	if(window.chrome){
		...
		clipboardClient.read(
			function(success,data){
				console.log(data);
				var weight = parseFloat(data);
				if(weight > 0){
					post_weight_data(tracking_number,weight);
				}else{
					alert('重量不在0-999999 之间！');
				}
				
			}
		);
		clipboardClient.write("0",function(success){console.log(success)});
	}else{
		alter('称重只支持chrome浏览器！');
		return;
	}
}
function post_weight_data(tracking_number,weight){
	var myurl = $("#WEB_ROOT").val()+"doShipWithWeight?tracking_number="+tracking_number+"&weight="+weight;
	var tracking_number = $.trim($('#tracking_number').val()); 
     ...
     $.ajax({
          	...
          	if (data != null ) {
          		if(data.status == 'ok'){
          			...
          			console.log('ok');
          			$('#tracking_number').val('');
          			$('#pre_tracking_number').html(tracking_number+'出库成功,重量：'+weight);
          		}else{
					play_audio_on_wrong_tracking_number();
          			console.log('error');
          			$('#tracking_number').val('');
          			$('#pre_tracking_number').html("快递面单号:"+tracking_number+data.error_info);
          			alert("快递面单号:"+tracking_number+data.error_info+",重量："+weight);
          		}
          	} else {
				play_audio_on_wrong_tracking_number();
          		$('#tracking_number').val('');
...
function get_clipboard_data() {
    if (window.clipboardData) {
        var weight = window.clipboardData.getData('Text');
