...
<br>
    <div class="container">
        ...
        <form style="width:100%;" method="post" action="<?php if(isset($WEB_ROOT))  echo $WEB_ROOT ; ?>./orderCount/query">
            <div class="row">
                <label for="shipping_start_time" class="col-sm-2 control-label">发货开始时间</label>
                <div class="col-sm-3">
                    <input required="required" autocomplete='off' type="text" class="form-control" name="shipping_start_time" id="shipping_start_time" value="<?php if(isset($shipping_start_time)) echo "{$shipping_start_time}"; ?>">
                </div>
                <label for="shipping_end_time" class="col-sm-2 control-label">发货结束时间</label>
                <div class="col-sm-3">
                    <input required="required" autocomplete='off' type="text" class="form-control" name="shipping_end_time" id="shipping_end_time" value="<?php if(isset($shipping_end_time)) echo "{$shipping_end_time}"; ?>">
                </div>
            </div>
            <div class="row">
            ...
            <div class="row check_select">
                ...
                <label>
                    <input type="checkbox" id="date_of_shipping"  name="date_of_shipping" value="1" 
                    <?php if (isset($date_of_shipping)) {
                        echo "checked";
                    } ?>> 发货日期
                </label>
                <label>
                    <input type="checkbox" id="hour_of_shipping"  name="hour_of_shipping" value="1" 
                    <?php if (isset($hour_of_shipping)) {
                        echo "checked";
                    } ?>> 发货时间
                </label>
                <label>
    ...
    <div class="container">
        <table class="table table-striped table-bordered" id="detail_table" style="width:100%;border: 3">
            <thead>
                <tr>
                <?php  $constant_array = array('area_name'=>'大区','facility_name'=>'仓库','date_of_shipping'=>'发货日期','hour_of_shipping'=>'发货时间','date_of_confirm'=>'成团日期','hour_of_confirm'=>'成团时间','province_name'=>'省','city_name'=>'市','district_name'=>'区','address_name'=>'公司/家庭','product_name'=>'product_name','goods_id'=>'goods_id','product_id'=>'product_id','shipping_name'=>'快递','count_of_shipment'=>'订单数');  ?>
                <?php  
                if (isset($orderCount_list) && is_array($orderCount_list) && !empty($orderCount_list)) {
                    foreach ($orderCount_list[0] as $key => $value) {
                        echo "<th>".$constant_array[$key]."</th>";
                    }
                }?>
                </tr>
            </thead>
            <?php if( isset($orderCount_list) && is_array($orderCount_list))  foreach ($orderCount_list as $key => $orderCount) { ?> 
            <tbody>
                <tr>
                    <?php
                        foreach($orderCount as $key=>$v){
                            if ($key=='hour_of_shipping'||$key=='hour_of_confirm') {
                                echo "<td>".$v." 点</td>";
                            }else{
                                echo "<td>".$v."</td>";
                            }
...
$(document).ready(function(){
        ...
        $("#shipping_start_time").calendar({btnBar:true,
                       minDate:'2010-05-01', 
                       maxDate:'2022-05-01'});
        $("#shipping_end_time").calendar({btnBar:true,
                       minDate:'2010-05-01', 
                       maxDate:'2022-05-01'});
...
$("#query").click(function(){
    $("#page_current").val("1");
    if ((!$("#shipping_start_time").val()||!$("#shipping_end_time").val())
            &&(!$("#confirm_start_time").val()||!$("#confirm_end_time").val())) {
        alert('请至少选择一组开始时间和结束时间');
        return false;
    };
