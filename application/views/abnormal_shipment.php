...
<div id="loadding"><img src="<?php if(isset($WEB_ROOT))  echo $WEB_ROOT ; ?>assets/img/loadding.gif"></div>
	<div class="container">
		...
		<div class="row  col-sm-10 " style="margin-top: 10px;">
			<p>1.商铺档案未设置</p>
            <table class="table table-striped table-bordered shop-table">
                <thead>
                    <tr>
                        <th> platform_id </th>
                        <th> platform_name </th>
	...
	function getShopFileUnset(){
		var postUrl = $("#WEB_ROOT").val() + 'AbnormalShipment/getShopFileUnset';
		var submit_data = {
			'abnormal_shipment_type': 'shop_file_unset'
		};
		$.ajax({
			url: postUrl,
			type: 'GET',
			...
			if (data.result == "ok") {
				...
				$.each(data.data,function(index,elem){
					html +="<tr><td>"+elem.platform_id+"</td><td>"+elem.platform_name+"</td><td>"+elem.shop_id+"</td><td>"+elem.shop_name+"</td><td><a target='_blank' href="+$('#WEB_ROOT').val()+"AbnormalShipment/getDetail?abnormal_shipment_type=shop_file_unset&shop_id="+elem.shop_id+">"+elem.num+"</a></td></tr>";
				});
